...
<div class="m-grid__item m-grid__item--fluid m-grid m-grid--ver-desktop m-grid--desktop m-body"><button id="m_aside_left_close_btn" class="m-aside-left-close m-aside-left-close--skin-dark"><i class="la la-close"></i></button>
    ...
    Ad name
    </label> <input placeholder="Ad name" name="adName" id="form-control-name" type="text" class="form-control m-input m-input--air"  required> <!----></div>     
    <div class="form-group m-form__group"><label for="form-control-name">
    Select Campaign 
    </label> 
    ...
    <select class="form-control" name="campaignAssign" onchange="showUser(this.value)">
     ...
     <!----></div> 
<label class="form-control-label">Specify certain hours or days of the week when your ads have to be shown</label> <div data-v-a6bdf47c="" class="form-group m-form__group"><!----> <!----> <div data-v-a6bdf47c=""></div>
    ...
    </label> <input min="0" id="form-control-daily_budget" name="dailyBudget" onchange="showReach(this.value)" type="number" class="form-control m-input m-input--air" required> <span class="m-form__help">Survey will not be limited by daily constraints</span><div class="mt-3" id="txtHintReach"></div> <!----></div></div></div></div></div></div> <div class="row"><div class="col"><div class="m-portlet"><div class="m-portlet__head m--bg-custom-light-blue"><div class="m-portlet__head-caption"><div class="m-portlet__head-title align-items-center"><span class="m-portlet__head-icon text-dark"><i class="la la-globe"></i></span> <h3 class="m-portlet__head-text text-dark">Geo Targeting</h3></div></div></div> <div class="d-flex justify-content-center mt-3"><button class="btn btn-primary mr-3" onclick="switchCountry()" type="button" >Multiple countries</button> <button class="btn ml-3 btn-primary" onclick="switchCountry()" type="button" >Single country</button></div>  <div class="m-portlet__body"><div class="m-form m-form--state"><div class="m-form__group form-group"><div><div class="btn-group btn-group-toggle w-100">
    ...
    Age To
     ...
     <div class="row form-group m-form__group align-items-center mx-0 row">
            <div class="col-6-sm">
                <p class="text-justify mx-auto mt-3 mb-3" style="width: 80%;">Please make sure that the size of your ad is not more than 500 MB. The file that you upload must be whit extension .mp4</p>
            </div>         
            <div class="col-6-sm">
                <input id="file1" class="inputfile1" type="file" id="myfile" name="files[]" multiple  required>
                <!--<label class="btn btn-prymary" for="file">Choose a file</label>-->
            </div>         
     </div>
...
function showReach(str) {
  if (str == "") {
    document.getElementById("txtHintReach").innerHTML = "";
    return;
  } else {
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        document.getElementById("txtHintReach").innerHTML = this.responseText;
      }
    };
    xmlhttp.open("GET","calculateReach.php?q="+str,true);
    xmlhttp.send();
  }
}
</script>
